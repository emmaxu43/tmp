...
void replace_blanks(char **story, catarray_t *cats) {
    size_t story_len = strlen(*story);
    size_t new_len = story_len;
    char *new_story = malloc(new_len + 1);
    if (new_story == NULL) {
        fprintf(stderr, "Memory allocation error\n");
        return;
    }
    strcpy(new_story, *story);

    char *blank_start = strstr(new_story, "_");
    category_t used_words = {NULL, 0, 0};

    while (blank_start != NULL) {
        char *blank_end = blank_start + 1;
        while (*blank_end != '_' && *blank_end != '\0') {
            blank_end++;
        }
        ...
        // Resize the story buffer if necessary
        if (replacement_len > blank_len) {
            size_t blank_start_offset = blank_start - new_story;
            new_len = new_len - blank_len + replacement_len;
            new_story = realloc(new_story, new_len + 1);
            if (new_story == NULL) {
                fprintf(stderr, "Memory allocation error\n");
                free(category_name);
                return;
            }
            blank_start = new_story + blank_start_offset;
            blank_end = blank_start + blank_len - 1;
        }

        // Move the remaining story content to make room for the replacement
        memmove(blank_start + replacement_len, blank_end + 1, new_story + new_len - (blank_end + 1));
        // Copy the replacement word into the blank space
        memcpy(blank_start, replacement, replacement_len);

        // Null-terminate the story string
        new_story[new_len] = '\0';
